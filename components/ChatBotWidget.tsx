...
const SYSTEM_INSTRUCTION = `You are the LeadFora AI assistant. You help agencies automate their sales using AI voice agents.
Your tone is helpful, professional, slightly enthusiastic, and concise.
Key Information:
- Pricing: Starts at $900/mo for a full Done-For-You setup. Zero headache, massive ROI.
- Integrations: Seamlessly connects with HubSpot, Zoho, Bayut, WhatsApp, Salesforce, Zapier.
- Languages: Fluent in 30+ languages including Arabic (Khaleeji/Darija), English, Russian, French, Spanish.
- Capability: You answer calls 24/7, qualify leads in real-time, and book meetings.
- Call to Action: If the user wants to test the AI, encourage them to click the 'Test AI Now' button or ask for the demo form.
Do not answer questions unrelated to LeadFora, sales automation, or AI agents.`;

const ChatBotWidget: React.FC<ChatBotWidgetProps> = ({ onOpenModal, isOpen, setIsOpen }) => {
  const [isTyping, setIsTyping] = useState(false);
  const [messages, setMessages] = useState<Message[]>([
    { id: 1, text: "Salam! 👋 I'm the LeadFora AI. I help agencies automate their sales. Ask me anything!", sender: 'ai' }
  ]);
      ...
      if (chatSession) {
        ...
        setMessages(prev => [...prev, aiMsg]);
         ...
         setTimeout(() => {
            ...
            const aiMsg: Message = { 
                id: Date.now() + 1, 
                text: "I'm connecting to the AI... please try again in a moment.", 
                sender: 'ai' 
            };
            setMessages(prev => [...prev, aiMsg]);
         }, 1000);
        ...
        >
          ...
          <div className="flex items-start gap-3">
            ...
            <p className="text-xs text-white font-medium leading-relaxed pr-4">
              Have questions about pricing or integration? Ask our AI Agent instantly. ⚡
            </p>
          </div>
        ...
        <div className="mb-4 w-[350px] h-[550px] flex flex-col bg-[#0F172A]/95 backdrop-blur-xl border border-cyan-500/30 rounded-2xl shadow-2xl overflow-hidden animate-fade-in-up origin-bottom-right transition-all duration-300">
          ...
          <div className="flex items-center justify-between p-4 bg-slate-900/80 border-b border-cyan-500/20 shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-white">LeadFora Assistant</h3>
                <span className="text-[10px] text-emerald-400 font-mono">● Online (Gemini Powered)</span>
              </div>
            </div>
            <button 
