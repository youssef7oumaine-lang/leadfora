...
const WhatsAppCard = () => (
  <div className="bg-[#E5DDD5] w-full h-full flex flex-col relative overflow-hidden rounded-[2rem] shadow-xl border border-slate-200">
    {/* WA Header */}
    <div className="bg-[#075E54] px-3 py-2 md:px-4 md:py-3 flex items-center justify-between shrink-0">
      <div className="flex items-center gap-2.5">
        <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-white flex items-center justify-center overflow-hidden p-1">
        ...
        <div>
          <h3 className="font-bold text-white text-[11px] md:text-xs">Wolfz AI (WA)</h3>
    ...
    {/* WA Messages */}
    <div className="flex-1 p-3 md:p-4 space-y-2 md:space-y-3 overflow-hidden relative">
      ...
      <div className="flex justify-start relative z-10">
        <div className="bg-white text-slate-800 px-3 py-1.5 rounded-lg rounded-tl-none shadow-sm max-w-[85%] text-[11px] md:text-[13px]">
          Hello! Here is the brochure 📄
          <span className="block text-[9px] text-slate-400 text-right mt-0.5">10:42 AM</span>
        </div>
      </div>
      <div className="flex justify-end relative z-10">
        <div className="bg-[#DCF8C6] text-slate-800 px-3 py-1.5 rounded-lg rounded-tr-none shadow-sm max-w-[85%] text-[11px] md:text-[13px]">
          Thanks, looking now.
    ...
    <div className="bg-[#F0F0F0] p-2 flex items-center gap-2 shrink-0">
       <div className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-white flex items-center justify-center text-slate-400 text-lg">+</div>
       <div className="flex-1 bg-white rounded-full h-7 md:h-8 px-3 text-xs text-slate-400 flex items-center">Type a message</div>
       <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-[#075E54] flex items-center justify-center text-white">
...
const InstagramCard = () => (
  <div className="bg-white w-full h-full flex flex-col relative overflow-hidden rounded-[2rem] shadow-xl border border-slate-200">
    {/* IG Header */}
    <div className="bg-gradient-to-r from-purple-600 to-orange-500 px-3 py-2 md:px-4 md:py-3 flex items-center justify-between shrink-0">
      <div className="flex items-center gap-2.5">
        <div className="w-7 h-7 md:w-8 md:h-8 rounded-full border-2 border-white flex items-center justify-center overflow-hidden">
           <img src="https://ui-avatars.com/api/?name=Wolfz+AI&background=random" alt="IG" className="w-full h-full object-cover" />
        </div>
        <div>
          <h3 className="font-bold text-white text-[11px] md:text-xs">wolfz.ai</h3>
    ...
    {/* IG Messages */}
    <div className="flex-1 p-3 md:p-4 space-y-2 md:space-y-3 overflow-hidden bg-white">
      <div className="flex justify-end">
        <div className="bg-[#3797F0] text-white px-3 py-2 rounded-[1rem] rounded-br-sm max-w-[85%] text-[11px] md:text-[13px]">
          Hi, is the Villa still available?
        </div>
      </div>
      <div className="flex justify-start">
         <div className="bg-slate-100 text-slate-800 px-3 py-2 rounded-[1rem] rounded-bl-sm max-w-[85%] text-[11px] md:text-[13px]">
    ...
    {/* IG Input */}
    <div className="p-2 md:p-3 bg-white border-t border-slate-100 flex items-center gap-2.5 shrink-0">
       <div className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-blue-500 flex items-center justify-center text-white">
       ...
       <div className="flex-1 bg-slate-100 rounded-full h-7 md:h-8 px-3 text-xs text-slate-400 flex items-center">Message...</div>
...
const WebChatCard = ({ messages, isTyping, showBadge, t }: any) => {
  ...
  return (
    <div className="bg-white rounded-[2rem] shadow-2xl shadow-slate-300 border border-slate-100 h-full flex flex-col relative overflow-hidden">
      {/* 1. Header */}
      <div className="shrink-0 flex items-center justify-between px-3 py-2 md:px-4 md:py-3 border-b border-slate-50 bg-white/90 backdrop-blur-md z-20">
        <div className="flex items-center gap-3">
            <div className="relative">
              <div className="w-8 h-8 md:w-9 md:h-9 rounded-full bg-gradient-to-br from-cyan-400 to-emerald-400 p-[2px]">
                  <div className="w-full h-full bg-slate-50 rounded-full flex items-center justify-center overflow-hidden">
                    <span className="text-[10px] md:text-sm font-bold text-transparent bg-clip-text bg-gradient-to-br from-cyan-500 to-emerald-500">AI</span>
                  </div>
              </div>
            ...
            <div>
              <h3 className="font-bold text-slate-900 text-[11px] md:text-xs leading-tight">Sarah</h3>
              <p className="text-[9px] md:text-[10px] text-slate-400 font-medium">Wolfz AI Agent</p>
            </div>
        </div>
      ...
      {/* 2. Message Area */}
      <div className="flex-1 p-2 md:p-4 flex flex-col bg-slate-50/50 overflow-y-auto no-scrollbar relative">
          <div className="mt-auto flex flex-col gap-3 md:gap-4">
            ...
            <AnimatePresence mode="popLayout">
              {messages.map((msg: any) => (
                ...
                >
                  <div 
                    className={`max-w-[90%] md:max-w-[85%] px-3 py-2 md:px-3.5 md:py-2.5 text-[11px] md:text-[13px] leading-relaxed shadow-sm ${
                      msg.type === 'user' 
      ...
      <AnimatePresence>
        {showBadge && (
          <div className="absolute bottom-16 md:bottom-20 left-4 right-4 flex items-center justify-center z-30">
            <motion.div
            ...
            >
                <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-emerald-100 flex items-center justify-center flex-shrink-0">
                  <svg className="w-3.5 h-3.5 md:w-4 md:h-4 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                  </svg>
                </div>
                <div>
                  <h4 className="font-bold text-slate-900 text-[11px] md:text-xs">{t.salesBot.badge}</h4>
                  <p className="text-[9px] md:text-[10px] text-slate-500">Synced to CRM. Agent notified.</p>
                </div>
            </motion.div>
          </div>
        )}
      ...
      {/* 4. Fake Input */}
      <div className="shrink-0 p-2 md:p-3 bg-white border-t border-slate-100 z-20">
        <div className="flex items-center gap-2.5">
            <div className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 cursor-default">
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
            </div>
            <div className="flex-1 bg-slate-50 border border-slate-100 rounded-full px-3 py-2">
              <span className="text-[11px] md:text-xs text-slate-400">Type a message...</span>
            </div>
            <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-gradient-to-r from-cyan-500 to-emerald-500 flex items-center justify-center text-white shadow-lg shadow-emerald-100 cursor-default">
